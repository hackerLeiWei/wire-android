...
class NotificationsHandlerService extends FutureService with ServiceHelper {
      ...
      case Some(accs) =>
        account match {
          case Some(acc) => accs.getZms(acc).flatMap {
            ...
            case Some(zms) =>
              verbose(l"Other device on account: $acc no longer active, resetting otherDeviceActiveTime")
              Future.successful(zms.notifications.otherDeviceActiveTime ! RemoteInstant.Epoch)
          }
          case None =>
  ...
  object NotificationsHandlerService {
    ...
    def checkNotificationsIntent(userId: UserId)(implicit context: Context): PendingIntent =
      PendingIntent.getService(
        context,
        userId.str.hashCode,
        new Intent(context, classOf[NotificationsHandlerService])
