...
class MessageNotificationsController(bundleEnabled: Boolean = Build.VERSION.SDK_INT > Build.VERSION_CODES.M,
  ...
  override def onNotificationsChanged(accountId: UserId, nots: Set[NotificationData], silent: Boolean): Future[Unit] = {
    verbose(l"onNotificationsChanged: $accountId, nots: $nots, silent: $silent")
    for {
      ...
      convNots  <- createConvNotifications(accountId, nots, teamName, silent).map(_.toMap)
      _         <- Threading.Ui {
  ...
  private def createConvNotifications(accountId: UserId, nots: Set[NotificationData], teamName: Option[Name], silent: Boolean) = {
    ...
    if (nots.nonEmpty) {
      ...
      Future.sequence(groupedConvs.filter(_._2.nonEmpty).map {
        case (notId, ns) =>
          for {
            commonProps   <- commonNotificationProperties(ns, accountId, silent = silent)
            specificProps <-
  ...
  private def commonNotificationProperties(ns: Seq[NotificationData], userId: UserId, silent: Boolean) =
    for {
  ...
  private def getSound(ns: Seq[NotificationData], silent: Boolean) = {
